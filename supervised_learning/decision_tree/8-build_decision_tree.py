...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Return the midpoints between consecutive distinct feature
        values seen at this node"""
        values = np.unique((self.explanatory[:, feature])[
            node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Find the threshold on the given feature that minimizes the
        average Gini impurity of the resulting children"""
        ...
        is_left = feature_values[:, None, None] > thresholds[None, :, None]
        is_class = target_values[:, None, None] == classes[None, None, :]

        left_counts = np.sum(is_left & is_class, axis=0)
        right_counts = np.sum(~is_left & is_class, axis=0)
        ...
        population_size = feature_values.size
        gini_average = (left_totals / population_size) * gini_left +\
            (right_totals / population_size) * gini_right

        best = np.argmin(gini_average)
        return thresholds[best], gini_average[best]

    def Gini_split_criterion(self, node):
        """Pick the feature and threshold minimizing the average Gini
        impurity of the resulting children"""
